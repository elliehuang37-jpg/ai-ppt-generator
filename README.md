...
**A. 讓 AI 幫你寫**
1. 選**簡報目的** → 2. 輸入**主題/方向** → 3. AI 產出**大綱**（可逐張編輯、排序、單張重寫）→ 4. 選**設計風格**（可套企業主色/Logo/封面版式）→ 5. **即時預覽 → 下載 PPTX**。

**B. 我已有文案**
1. 首頁點「**📄 我已有文案，直接匯入**」→ 2. 貼上文字或上傳 **.txt / .md / .docx** → 3. 按「**直接轉成投影片**」（免金鑰）或「**用 AI 整理**」→ 4. 步驟 3 會出現**智慧建議**（哪幾頁該用圖表/KPI），按「**一鍵套用**」自動優化並跳到預覽 → 5. 選風格 → 下載。

> 隨時可在預覽頁按「✏️ 回到編輯大綱」繼續修改；進度會自動存在瀏覽器。
